...
If you choose to deploy special data adapters, the Quick Start will deploy the following additional resources. They are depicted in <<architecture_adapters1>> in the "Modular CloudFormation adapter substack" group (shaded).

* An AWS Lambda function to retrieve MRASCo data files from an S3 bucket, transform them to MDA Landing zone format, and send them to a data lake.
* An S3 bucket to store MRASCo data files.
