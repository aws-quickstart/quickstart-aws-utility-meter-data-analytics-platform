...
WARNING: Make sure that your landing-zone S3 bucket contains only meter-data files. If it contains any subfolders, the ETL workflow will fail.
...
This Quick Start requires geolocation data to be uploaded to the `geodata` folder in the business-zone S3 bucket. You can get the name of this bucket from the output section in the CloudFormation stack. The S3 path of geodata location should be `s3://<business-zone-s3-bucket-name>/geodata/`. The data should be in a .CSV file format with the following structure, where all three columns (meter id, latitude, longitude) are mandatory with comma-separated values.

//TODO Marcia, maybe the table is misleading. The table describes the schmea fields meter_id, lat, long which are the three mentioned columns
