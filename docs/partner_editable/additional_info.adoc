...
This Quick Start requires geo-location data to be uploaded to the _geodata_ folder in Landing Zone S3 bucket. The S3 path of this location should be _s3://<Landing-zone-s3-bucket-name>/geodata/_. Geo-location data should be in a .CSV file format with the following structure, where all 3 columns are mandatory with comma-separated values.
