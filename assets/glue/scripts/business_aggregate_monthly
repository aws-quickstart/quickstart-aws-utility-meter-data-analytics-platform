...
business_zone_bucket_path_monthly = "s3://" + args['business_zone_bucket'] + "/aggregated/monthly"

glueContext.write_dynamic_frame.from_options(frame = dfAggregatedReads, \
    connection_type = "s3", \
    connection_options = {"path": business_zone_bucket_path_monthly, "partitionKeys": ["year", "month"]},\
    format = "parquet", \
