...
aggregatedReads = cleanedMeterDataSource.toDF() \
    .filter("reading_type = 'INT'") \
    .groupby('meter_id', 'week_of_year', 'month', 'year') \
    .agg(sum("reading_value").alias("Sum Reading Value")) 

dfAggregatedReads = DynamicFrame.fromDF(aggregatedReads, glueContext, "dfAggregatedReads")

business_zone_bucket_path_weekly = "s3://" + args['business_zone_bucket'] + "/aggregated/weekly"

glueContext.write_dynamic_frame.from_options(frame = dfAggregatedReads, \
    connection_type = "s3", \
    connection_options = {"path": business_zone_bucket_path_weekly, "partitionKeys": ["year", "week_of_year"]},\
    format = "parquet", \
